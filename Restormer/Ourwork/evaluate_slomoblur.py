...
parser.add_argument('--output_dir', default='./Dataset/SloMoBlur/0_9999/deblurred', type=str, 
...
parser.add_argument('--batch_size', type=int, default=4,
                    help='Batch size for processing (default: 32). With 224x224 images, can use larger batches to keep GPU busy.')
parser.add_argument('--num_workers', type=int, default=4,
                    help='Number of workers for data loading (default: 4)')
parser.add_argument('--resize', type=int, nargs=2, default=[224, 224], metavar=('H', 'W'),
                    help='Resize images to this size before processing (default: 224 224). Images will be restored to original size before saving.')
...
class ImageDataset(data.Dataset):
    ...
    def __init__(self, file_paths, img_multiple_of=8, resize_to=(224, 224)):
        self.file_paths = file_paths
        self.img_multiple_of = img_multiple_of
        self.resize_to = resize_to  # (height, width)
    ...
    def __getitem__(self, idx):
        ...
        blurred_np = cv2.resize(blurred_np, (self.resize_to[1], self.resize_to[0]), interpolation=cv2.INTER_LINEAR)
        ...
        return {
            'image': input_tensor,
            'file_path': file_path,
            'original_height': original_height,  # Store original size
            'original_width': original_width,   # Store original size
            'resized_height': height,          # Store resized dimensions
            'resized_width': width,             # Store resized dimensions
...
def collate_fn(batch):
    ...
    if len(batch) == 1:
        ...
        return {
            ...
            'resized_heights': [item['resized_height']],
            'resized_widths': [item['resized_width']],
    ...
    for item in batch:
        ...
        resized_heights.append(item['resized_height'])
        resized_widths.append(item['resized_width'])
...
print(f"Resizing images to: {args.resize[0]}x{args.resize[1]} before processing")
print(f"Images will be restored to original size before saving")
...
dataset = ImageDataset(files, img_multiple_of=img_multiple_of, resize_to=tuple(args.resize))
...
with torch.no_grad():
    ...
    for batch_idx, batch in enumerate(tqdm(dataloader, desc="Processing images")):
        try:
            ...
            resized_heights = batch['resized_heights']
            resized_widths = batch['resized_widths']
            ...
            for i in range(batch_size):
                try:
                    # Unpad the output to resized size (remove padding) - similar to demo.py
                    resized_h = resized_heights[i]
                    resized_w = resized_widths[i]
                    restored_img = restored[i:i+1, :, :resized_h, :resized_w]
                    ...
                    if resized_h != original_h or resized_w != original_w:
                        # Need to resize - do it on tensor first, then convert
                        restored_img = F.interpolate(
                            restored_img, 
                    ...
                    # Convert to numpy following demo.py approach
                    restored_np = restored_img.permute(0, 2, 3, 1).cpu().detach().numpy()
                    
                    # Normalize values to [0, 1] range instead of clipping
                    restored_np_img = restored_np[0]
                    min_val = restored_np_img.min()
                    max_val = restored_np_img.max()
                    if max_val > min_val:
                        # Normalize: (x - min) / (max - min) to map to [0, 1]
                        restored_np_img = (restored_np_img - min_val) / (max_val - min_val)
                    elif max_val == min_val and max_val != 0:
                        # If all values are the same non-zero, keep them normalized
                        restored_np_img = np.ones_like(restored_np_img) if max_val > 0 else np.zeros_like(restored_np_img)
                    # If all zeros, keep as is
                    
                    # Convert to uint8
                    restored_uint8 = (restored_np_img * 255.0).astype(np.uint8)
